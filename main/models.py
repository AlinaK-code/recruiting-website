...
email_validator = EmailValidator(message="Введите корректный email адрес")
...
class Company(models.Model):
    ...
    logo = models.ImageField(
        upload_to="companies/logos/", blank=True, null=True, verbose_name="Логотип"
    )
...
class CandidateProfile(models.Model):
    ...
    phone = models.CharField(
        max_length=20, blank=True, default='', validators=[phone_regex], verbose_name="Телефон"
    )
    contact_email = models.EmailField(
        validators=[email_validator], verbose_name="Контактный email", default='user@example.com'
    )
    avatar = models.ImageField(
        upload_to="candidates/avatars/", blank=True, null=True, verbose_name="Аватар"
    )
    ...
    skills = models.ManyToManyField(  # 1 пользователь может иметь много навыков
        Skill, blank=True, verbose_name="Навыки"
    )
...
class Vacancy(models.Model):
    ...
    skills = models.ManyToManyField(Skill, blank=True, verbose_name="Требуемые навыки")
    contact_email = models.EmailField(
        validators=[email_validator], verbose_name="Email для связи", default='hr@company.com',
    )
    contact_phone = models.CharField(
        max_length=20, validators=[phone_regex], verbose_name="Телефон для связи", default='+70000000000'
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Создана")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Обновлена")
    ...
    class Meta:
        ...
        ordering = ["-created_at"]
        # добавляю ограниичения для уникальности вакансий для работодателя
        # чтобы нельзя было создавать две вакансии с одинаковым названием и описанием у одной компании
        constraints = [
            models.UniqueConstraint(
                fields=["title", "description", "company"],
                name="unique_vacancy_per_company",
            )
        ]
...
class Application(models.Model):
    ...
    class Meta:
        ...
        unique_together = (
            "candidate",
            "vacancy",
        )  # один кандидат — один отклик на вакансию
...
class Interview(models.Model):
    ...
    application = models.OneToOneField(
        Application, on_delete=models.CASCADE, verbose_name="Отклик"
    )
    ...
    format = models.CharField(
        max_length=10, choices=FORMAT_CHOICES, default="online", verbose_name="Формат"
    )
...
class Feedback(models.Model):
    interview = models.OneToOneField(
        Interview, on_delete=models.CASCADE, verbose_name="Собеседование"
    )
    rating = models.PositiveSmallIntegerField(
        choices=[(i, i) for i in range(1, 6)], verbose_name="Оценка (1-5)"
    )
